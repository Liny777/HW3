...
public class PointC implements Writable {
...
//    private int maxLabel; // 当前cluster里label数量最多的label
    ...
    public PointC(final float[] c, int totalType, int centerClass, int pnumPoints, int pownLabel, final int[] e) {
...
//    }
    ...
    public PointC(final String[] s, int totalType) {
        this.set(s,totalType);
    }

    public static PointC copy(final PointC p) {
        PointC ret = new PointC(p.components,p.typeClass,p.classLabel,p.numPoints,p.ownLabel,p.eachLabelNum);
        return ret;
    }
...
//        this.ownLabel = Integer.parseInt(s[0]);
    ...
    public float distance(PointC p){
        // Euclidean
        float dist = 0.0f;
        for (int i = 0; i < this.dim; i++) {
            // 每个维度进行做差
