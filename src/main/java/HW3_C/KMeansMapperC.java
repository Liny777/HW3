...
public class KMeansMapperC extends Mapper<LongWritable, Text, IntWritable, PointC> {

    private PointC[] centroids;
    private final PointC point = new PointC(); // 数据集里的每一个数据点
    ...
    protected void setup(Context context) throws IOException, InterruptedException {
        ...
        this.centroids = new PointC[k];
        for(int i = 0; i < k; i++) {
            String[] centroid = context.getConfiguration().getStrings("centroid." + i);
            this.centroids[i] = new PointC(centroid,k);
